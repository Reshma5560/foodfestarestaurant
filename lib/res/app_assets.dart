class AppAssets {
  static const String iconPath = "assets/icons/";
  static const String imagesPath = "assets/images/";
  static const String svgPath = "assets/svg/";
  static const String rivePath = "assets/rive/";

  /// ICONS ///
  static String infoProfileIcon = '${iconPath}info_profile_icon.png';
  
  static String homeIcon = '${iconPath}ic_home.png';
  static String categoryIcon = '${iconPath}ic_category.png';
  static String profileIcon = '${iconPath}ic_profile.png';

  /// IMAGES ///
  static String appLogo = '${imagesPath}app_logo.png';
  static String userImage = '${imagesPath}user_image.jpeg';
  static String emptyData = '${imagesPath}empty_data.png';
  static String notificationNotFond = '${imagesPath}notification_not_fond.png';
  static String searchDataEmpty = '${imagesPath}search_empty.png';

  /// SVG ///

  static String backArrowIcon = '${svgPath}back_arrow_svg.svg';
  static String notificationsIcon = '${svgPath}notifications_icon.svg';
  static String googleIcon = '${svgPath}google.svg';
  static String facebookIcon = '${svgPath}facebook.svg';

  /// reactions
  static String astonishedReaction = '${svgPath}astonished_reaction.svg';
  static String thumbsUpReaction = '${svgPath}thumbs_up_reaction.svg';
  static String thumbsDownReaction = '${svgPath}thumbs_down_reaction.svg';
  static String heartEyesReaction = '${svgPath}heart_eyes_reaction.svg';
}
